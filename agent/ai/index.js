...
export const ACTION_TOOLS = [
  ...
  },
  {
    ...
    description: 'Phase 5A: Creates a local outreach EMAIL draft from a READY proposal and the prospect\'s verified public contact email. Does NOT send email, submit forms, contact anyone, or set CONTACTED. Fails if no verified public email is stored on the prospect. Never invents recipient addresses.',
    parameters: {
      type: 'object',
      properties: {
        opportunityId: { type: 'string', description: 'ID of the opportunity (must be AWAITING_APPROVAL or ready for outreach prep).' },
        proposalId: { type: 'string', description: 'ID of a READY proposal for that opportunity.' },
        channel: { type: 'string', description: 'Outreach channel. Phase 5A supports only "email".' },
      },
