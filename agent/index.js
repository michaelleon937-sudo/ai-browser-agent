  ...
  try {
    while (true) {
      ...
      if (execResult.ok) {
        ...
        dbErrors.record({ runId: run.id, stepId: stepRow.id, level: 'error', message: execResult.error, context: { tool: action.tool, args: redact(action.args), permanent: !!execResult.permanent } });
        ...
        if (!execResult.permanent) state.retriesTotal++;
      }
    }
...
async function executeWithRetry(action, { stepTimeoutMs, maxRetries, context }) {
  ...
  for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
    try {
      ...
      lastErr = err;
      if (classifyError(err.message) === 'permanent') return { ok: false, error: err.message, permanent: true };
